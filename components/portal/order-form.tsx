...
import { UploadCloud, X, File as FileIcon, Loader2, Book, Printer, ChevronLeft, RefreshCcw, MapPin, Truck, Store, Search } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import { toast } from 'sonner'
import { useRouter } from 'next/navigation'
import { submitOrder } from '@/app/portal/actions'
...
const SUBJECTS = [
    "اللغة العربية", "اللغة الإنجليزية", "رياضيات", "علوم", "دراسات اجتماعية",
    "دين", "حاسب آلي", "تربية فنية", "اخرى"
]
...
export function PortalOrderForm({ userId, settings }: { userId: string, settings?: SettingsMap }) {
    ...
    const selectType = (type: 'notebook' | 'print') => {
        setOrderType(type)
        if (type === 'notebook') {
            setItems([{ type: 'notebook', subject: '', stage: '', grade: '', quantity: 1, paperCount: 0 }])
        } else {
            setItems([{ type: 'print', quantity: 1, pageCount: 0, binding: false, colorMode: 'bw', printSide: 'one', file: null, additionalNotes: '' }])
        }
    }

    const resetType = () => {
        if (confirm("هل أنت متأكد من تغيير نوع الطلب؟ سيتم حذف البيانات الحالية.")) {
            setOrderType(null)
            setItems([])
        }
    }
    ...
    const getLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                setDeliveryAddress(`الموقع: ${latitude.toFixed(6)}, ${longitude.toFixed(6)}`)
                toast.success('تم الحصول على موقعك بنجاح!')
                setGettingLocation(false)
            },
            (error) => {
                toast.error('فشل الحصول على الموقع. يرجى التأكد من السماح بالوصول للموقع')
                setGettingLocation(false)
            }
        )
    }
    ...
    const addItem = () => {
        if (orderType === 'notebook') {
            setItems(prev => [...prev, { type: 'notebook', subject: '', stage: '', grade: '', quantity: 1, paperCount: 0 }])
        } else {
            setItems(prev => [...prev, { type: 'print', quantity: 1, pageCount: 0, binding: false, colorMode: 'bw', printSide: 'one', file: null, additionalNotes: '' }])
        }
    }
        ...
        if (orderType === 'print') {
            items.forEach((item: any) => {
                if (item.type === 'print') {
                    ...
                    let bindingCost = 0
                    if (item.binding) {
                        const singleCopyBindingCost = sheets < prices.binding_threshold ? prices.binding_small : prices.binding_large
    ...
    async function handleSubmit(formData: FormData) {
        ...
        try {
            // Validate items
            if (activeItems.some(i => {
                if (i.type === 'notebook') return !i.subject || !i.stage || !i.grade
                if (i.type === 'print') return !i.pageCount || i.pageCount < 1 || !i.file
                return false
            })) {
            ...
            const submissionItems = await Promise.all(activeItems.map(async (item) => {
                let itemFileUrl = ''
                ...
                if (item.type === 'print' && item.file) {
                    ...
                    const { error } = await supabase.storage.from('order-files').upload(fileName, file)

                    if (error) throw new Error(`Failed to upload ${file.name}`)

                    const { data: { publicUrl } } = supabase.storage.from('order-files').getPublicUrl(fileName)
                ...
                if (item.type === 'print') {
                    const notes = item.additionalNotes ? ` - ملاحظات: ${item.additionalNotes}` : ''
                    return {
                        ...item,
            ...
            <Card className="border-none shadow-lg max-w-3xl mx-auto mt-10">
                ...
                <CardContent className="grid md:grid-cols-2 gap-6 p-8">
                    ...
                    >
                        ...
                        <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">دفاتر تحضير</h3>
                        <p className="text-center text-sm text-gray-600 dark:text-gray-400">طباعة وتجهيز مذكرات مع ورق تحضير للمدرسين</p>
                    </div>
                    ...
                    >
                        ...
                        <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">طباعة ملفات</h3>
                        <p className="text-center text-sm text-gray-600 dark:text-gray-400">طباعة مستندات، ملازم، صور من ملفاتك</p>
                    </div>
                </CardContent>
            </Card>
        )
    ...
    return (
        <>
            <form onSubmit={(e) => {
                ...
                <Card className="border-none shadow-lg">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <CardTitle className="flex items-center gap-2">
                                    {orderType === 'notebook' ? <Book className="w-5 h-5 text-orange-500" /> : <Printer className="w-5 h-5 text-blue-500" />}
                                    {orderType === 'notebook' ? 'طلب دفاتر تحضير' : 'طلب طباعة ملفات'}
                                </CardTitle>
                                <CardDescription>قم بتعبئة البيانات المطلوبة</CardDescription>
                            </div>
                        </div>
                        <Button variant="ghost" size="sm" onClick={resetType} type="button" className="text-muted-foreground hover:text-red-500">
                    ...
                    <CardContent className="space-y-8">
                        ...
                        <div className="space-y-4">
                            <div className="space-y-3">
                                {activeItems.map((item, index) => (
                                    <div key={index} className="p-4 bg-card text-card-foreground border shadow-sm rounded-xl relative group">
                                        ...
                                        {item.type === 'notebook' ? (
                                            <div className="grid grid-cols-12 gap-4">
                                                ...
                                                <div className="col-span-12 sm:col-span-4 space-y-2">
                                                    ...
                                                    <Select key={`subject-${index}`} value={item.subject} onValueChange={(val) => updateItem(index, 'subject', val)}>
                                                        <SelectTrigger className="font-bold text-base h-10 border-2 focus:ring-emerald-500 text-right w-full">
                                                            <SelectValue placeholder="اختر المادة" />
                                                        </SelectTrigger>
                                                        <SelectContent>
                                                            {SUBJECTS.map(s => <SelectItem key={s} value={s} className="text-base font-bold">{s}</SelectItem>)}
                                                        </SelectContent>
                                                    </Select>
                                                </div>
                                                ...
                                                <div className="col-span-6 sm:col-span-4 space-y-2">
                                                    ...
                                                    <Select key={`stage-${index}`} value={item.stage} onValueChange={(val) => updateItem(index, 'stage', val)}>
                                                        <SelectTrigger className="font-bold text-base h-10 border-2 focus:ring-emerald-500 text-right w-full">
                                                            <SelectValue placeholder="اختر المرحلة" />
                                                        </SelectTrigger>
                                                        <SelectContent>
                                                            {STAGES.map(s => <SelectItem key={s.id} value={s.id} className="text-base font-bold">{s.label}</SelectItem>)}
                                                        </SelectContent>
                                                    </Select>
                                                </div>
                                                ...
                                                <div className="col-span-6 sm:col-span-4 space-y-2">
                                                    ...
                                                    <Select
                                                        key={`grade-${index}`}
                                                        value={item.grade}
                                                        onValueChange={(val) => updateItem(index, 'grade', val)}
                                                        disabled={!item.stage}
                                                    >
                                                        <SelectTrigger className="font-bold text-base h-10 border-2 focus:ring-emerald-500 text-right w-full">
                                                        ...
                                                        <SelectContent>
                                                            {item.stage && GRADES[item.stage as keyof typeof GRADES]?.map(g => (
                                                                <SelectItem key={g} value={g} className="text-base font-bold">{g}</SelectItem>
                                                            ))}
                                                        </SelectContent>
                                                    </Select>
                                                </div>
                                                ...
                                                <div className="col-span-6 sm:col-span-3 space-y-2">
                                                    <Label className="text-base font-bold">الورق الصافي للتحضير</Label>
                                                    <Input
                                                        type="number"
                                                        min="0"
                                                        placeholder="0"
                                                        value={item.paperCount || ''}
                                                        onChange={(e) => updateItem(index, 'paperCount', parseInt(e.target.value) || 0)}
                                                        className="text-center text-lg font-bold border-2 focus-visible:ring-emerald-500 h-10"
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-12 gap-2 items-end">
                                                ...
                                                <div className="col-span-2 md:col-span-12 mb-2">
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <UploadCloud className={`w-5 h-5 ${item.file ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'}`} />
                                                            <div className="text-right">
                                                                <p className={`text-base font-bold ${item.file ? 'text-emerald-800 dark:text-emerald-300' : 'text-slate-700 dark:text-slate-200'}`}>
                                                                    {item.fileName || 'اختر ملف للطباعة'}
                                                                </p>
                                                                {!item.file && <p className="text-xs font-bold text-slate-400 dark:text-slate-500">PDF, Word, Images</p>}
                                                            </div>
                                                        </div>
                                                        {item.file && (
                                                            <Button
                                                ...
                                                <div className="col-span-1 md:col-span-2 space-y-1.5">
                                                    <Label className="text-base font-bold text-slate-700 dark:text-slate-200">عدد الصفحات</Label>
                                                    <Input
                                                        type="number"
                                                        min="1"
                                                        placeholder="0"
                                                        value={item.pageCount || ''}
                                                        onChange={(e) => updateItem(index, 'pageCount', parseInt(e.target.value) || 0)}
                                                        required
                                                ...
                                                {/* Copies */}
                                                <div className="col-span-1 md:col-span-2 space-y-1.5">
                                                    <Label className="text-base font-bold text-slate-700 dark:text-slate-200">نسخ</Label>
                                                    <Input
                                                        type="number"
                                                        min="1"
                                                        value={item.quantity}
                                                ...
                                                {/* Color Mode */}
                                                <div className="col-span-1 md:col-span-2 space-y-1.5">
                                                    <Label className="text-base font-bold text-slate-700 dark:text-slate-200">النظام</Label>
                                                    <select
                                                        value={item.colorMode}
                                                        onChange={(e) => updateItem(index, 'colorMode', e.target.value)}
                                                ...
                                                <div className="col-span-1 md:col-span-3 space-y-1.5">
                                                    <Label className="text-base font-bold text-slate-700 dark:text-slate-200">خيارات الطباعة</Label>
                                                    <select
                                                        value={item.printSide}
                                                        onChange={(e) => updateItem(index, 'printSide', e.target.value)}
                                                        className="font-bold text-base h-10 border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-white focus:ring-blue-600 text-right px-2 w-full rounded-md border bg-white"
                                                    >
                                                ...
                                                {/* Binding Checkbox */}
                                                <div className="col-span-1 md:col-span-3 space-y-1.5">
                                                    <Label className="text-base font-bold text-slate-700 dark:text-slate-200">التكعيب</Label>
                                                    <div className="flex items-center justify-center gap-3 h-10 border-2 border-slate-300 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 hover:border-blue-500 dark:hover:border-blue-400 transition-colors">
                                                        <input
                                                            ...
                                                            onChange={(e) => updateItem(index, 'binding', e.target.checked)}
                                                            className="h-5 w-5 rounded-md border-2 border-slate-400 cursor-pointer accent-blue-600"
                                                        />
                                                        <Label htmlFor={`binding-${index}`} className="cursor-pointer text-base font-bold text-slate-800 dark:text-slate-200 select-none">مطلوب</Label>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <Checkbox
                                                            ...
                                                            onCheckedChange={(checked) => {
                                                                if (!checked) updateItem(index, 'additionalNotes', '')
                                                        ...
                                                        />
                                                        <Label htmlFor={`notes-check-${index}`} className="text-base cursor-pointer text-slate-600 dark:text-slate-300 hover:text-amber-700 dark:hover:text-amber-500 transition-colors select-none font-bold">
                                                            إضافة ملاحظات خاصة؟
                                                        </Label>
                                                    </div>

                                                    {item.additionalNotes !== undefined && item.additionalNotes !== '' && (
                                                        <Input
                                                            placeholder="تفاصيل التكعيب أو ملاحظات أخرى..."
                                                            value={item.additionalNotes.trim() === '' && item.additionalNotes.length === 1 ? '' : item.additionalNotes}
                                                            onChange={(e) => updateItem(index, 'additionalNotes', e.target.value)}
                            ...
                            >
                                {orderType === 'notebook' ? <Book className="w-4 h-4" /> : <Printer className="w-4 h-4" />}
                                {orderType === 'notebook' ? 'إضافة دفتر آخر' : 'إضافة ملف طباعة آخر'}
                            </Button>
                        </div>
                            ...
                            <div className="space-y-2">
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="text-center">
                                            <p className="text-sm font-medium text-gray-700 dark:text-gray-300">اضغط لرفع الملفات</p>
                                            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">PDF, Images, Word</p>
                                        </div>
                                    </div>
                                    <input
                                        type="file"
                                ...
                                {files.length > 0 && (
                                    <div className="grid gap-2 mt-2">
                                        {files.map((file, i) => (
                                            <div key={i} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-slate-800/50 rounded-lg border dark:border-slate-700 text-sm">
                                                <span className="truncate max-w-[200px] dark:text-slate-200">{file.name}</span>
                                                <Button
                                                    type="button"
                                                    variant="ghost"
                                                    size="sm"
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="notes" className="text-base font-bold">ملاحظات إضافية</Label>
                            <Textarea
                                id="notes"
                                name="notes"
                        ...
                        <div className="space-y-4 p-4 bg-blue-50/50 dark:bg-blue-950/20 rounded-lg border-2 border-blue-200 dark:border-blue-900">
                            <Label className="text-base font-bold text-slate-700 dark:text-slate-200">طريقة الاستلام</Label>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                >
                                    <div className="flex flex-col items-center gap-2 text-center">
                                        <Store className={`w-8 h-8 ${deliveryMethod === 'pickup' ? 'text-emerald-600' : 'text-slate-500'}`} />
                                        <span className={`font-bold ${deliveryMethod === 'pickup' ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-700 dark:text-slate-300'}`}>
                                            استلام من المحل
                                        </span>
                                        <span className="text-xs text-slate-500 dark:text-slate-400">مجاني</span>
                                    </div>
                                </div>
                                ...
                                >
                                    <div className="flex flex-col items-center gap-2 text-center">
                                        <Truck className={`w-8 h-8 ${deliveryMethod === 'delivery' ? 'text-blue-600' : 'text-slate-500'}`} />
                                        <span className={`font-bold ${deliveryMethod === 'delivery' ? 'text-blue-700 dark:text-blue-400' : 'text-slate-700 dark:text-slate-300'}`}>
                                            توصيل للعنوان
                                        </span>
                                        <span className="text-xs text-orange-600 dark:text-orange-400 font-semibold">+30-50 جنيه</span>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="space-y-3 p-4 bg-white dark:bg-slate-900 rounded-lg border dark:border-slate-700">
                                    <div className="flex items-start gap-2 p-3 bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-900 rounded-lg">
                                        <span className="text-orange-600 dark:text-orange-400 text-sm">
                                            ℹ️ <strong>ملحوظة:</strong> سيتم إضافة تكلفة الشحن (30-50 جنيه) على حسب المسافة
                                        </span>
                                    </div>

                                    <div className="space-y-2">
                                        <Label htmlFor="address" className="text-sm font-bold">العنوان التفصيلي</Label>
                                        <Textarea
                                            id="address"
                                    ...
                                    {deliveryLocation && (
                                        <div className="flex gap-2">
                                            <Button
                                                ...
                                                onClick={() => window.open(`https://www.google.com/maps?q=${deliveryLocation.lat},${deliveryLocation.lng}`, '_blank')}
                                            >
                                                عرض على الخريطة
                                            </Button>
                                        </div>
                        ...
                        {orderType === 'print' && (
                            <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border dark:border-slate-700">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="font-bold text-lg dark:text-slate-200">حساب التكلفة التقديرية</h3>
                                    <Button
                                        type="button"
                                        variant="outline"
                                ...
                                {estimatedTotal !== null && (
                                    <div className="text-center p-3 bg-white dark:bg-slate-950 rounded border-2 border-emerald-500/50 dark:border-emerald-500/30">
                                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">الإجمالي المتوقع</p>
                                        <p className="text-3xl font-bold text-emerald-600 dark:text-emerald-400">
                                            {estimatedTotal} <span className="text-base font-normal">جنيه</span>
                                        </p>
                                        <p className="text-xs text-slate-400 mt-2 dark:text-slate-500">* التكلفة نهائية قد تختلف قليلاً عند المراجعة</p>
                                    </div>
                                )}
                            </div>
                        )}
            ...
            {/* Map Picker Modal */}
            {showMapPicker && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4" onClick={() => setShowMapPicker(false)}>
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
                        {/* Modal Header */}
                        <div className="p-4 border-b dark:border-slate-700 flex items-center justify-between bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-950/30 dark:to-blue-950/30">
                            <div>
                                <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">اختر موقع التوصيل</h3>
                                <p className="text-sm text-slate-600 dark:text-slate-400">انقر على الخريطة لتحديد المكان</p>
                            </div>
                            <Button
                                type="button"
                                variant="ghost"
                        ...
                        <div className="p-3 md:p-4 space-y-3 md:space-y-4 overflow-y-auto max-h-[calc(90vh-80px)]">
                            ...
                            {deliveryLocation && (
                                <div className="bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 rounded-lg p-3 text-sm">
                                    <p className="font-bold text-green-700 dark:text-green-400 mb-1">✓ تم تحديد الموقع</p>
                                    <p className="text-green-600 dark:text-green-500 text-xs">{deliveryAddress}</p>
                                </div>
                            )}
